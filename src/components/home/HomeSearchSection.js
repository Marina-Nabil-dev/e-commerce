  ...
  return (
    <>
      <div className="flex flex-col items-center p-[250px_80px_40px_80px] justify-center">
        <h1 className="pb-2 text-5xl font-bold  ">Welcome to Biddex</h1>
        <h2 className="text-2xl font-semibold items-center w-auto">
          We are a platform that is user-friendly, making it easy to sell or buy
          a car without any hassle. Additionally, we provide you with a variety
          of sales, including auction sales.
        </h2>
      </div>
      ...
      <form className="relative flex items-start justify-center m-[26px_80px_26px_80px] text-gray-500 p-[10px_10px_40px_10px] gap-5 rounded-[10px] border-1	">
        ...
        <div className="flex flex-col space-y-2 bg-white text-gray-500 rounded-[10px] border-2">
          ...
          >
            ...
            {sellTypes.map((saleType) => (
              <option key={saleType.key} value={saleType.slug}>
                {saleType.name}
              </option>
            ))}
          </select>
        ...
        <button
          type="submit"
          className="flex items-center px-4 py-2 bg-primary text-white rounded-full font-bold border-1"
        >
